-- Function: doc_orders_before_open(varchar(32), integer)

-- DROP FUNCTION doc_orders_before_open(varchar(32), integer);

CREATE OR REPLACE FUNCTION doc_orders_before_open(in_view_id varchar(32), in_doc_id integer)
  RETURNS void AS
$BODY$
BEGIN
	...
	IF (in_doc_id IS NOT NULL AND in_doc_id>0) THEN
		...
		(view_id,line_number,product_id,quant,quant_confirmed,quant_base_measure_unit,quant_confirmed_base_measure_unit,volume,weight,price,price_edit,total,total_pack,mes_length,mes_width,mes_height,measure_unit_id,pack_exists,pack_in_price)
		(SELECT in_view_id
	...
	IF (in_doc_id IS NOT NULL AND in_doc_id>0) THEN
		...
		(view_id,line_number,customer_survey_question_id,points,answer_comment)
		(SELECT in_view_id
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION doc_orders_before_open(varchar(32), integer)
