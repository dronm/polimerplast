	...
	*/	
	...
	try{		
		...
		if ($com==CMD_ADD_CLIENT){
			...
			ВЫРАЗИТЬ( СУММА(ЕСТЬNULL(БухОст.СуммаОстатокДт,0)) - СУММА(ЕСТЬNULL(БухОст.СуммаОстатокКт,0)) КАК ЧИСЛО(15,2)) AS Сумма
			...
			ГДЕ БухОст.СуммаОстатокДт<>0 ИЛИ БухОст.СуммаОстатокКт<>0
