...
//******************************************************************************	
	...
	function get_person_attr_query(){
		...
		ПО ДопВодУд.Ссылка=Спр.Ссылка И ДопВодУд.Свойство В (
					...
					ГДЕ НаборСвойств.Наименование = "Физические лица"
					//ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.Справочник_ФизическиеЛица)
	...
	function get_firm_bank_account_list($v8,$firmExtId){
		$firm_id = $v8->NewObject('УникальныйИдентификатор',$firmExtId);
		$firm_ref = $v8->Справочники->Организации->ПолучитьСсылку($firm_id);			
		
		$q_obj = $v8->NewObject('Запрос');
		$q_obj->Текст ='ВЫБРАТЬ Ссылка,Наименование ИЗ Справочник.БанковскиеСчетаОрганизаций
		ГДЕ Владелец=&firm И Не Закрыт И НЕ ПометкаУдаления';
		$q_obj->УстановитьПараметр('firm',$firm_ref);		
		$sel = $q_obj->Выполнить()->Выбрать();
		$xml = '';
		while ($sel->Следующий()){
...
//throw new Exception('UserId='.$attrs['user_ext_id']);
		if (is_null($client_ref)){
			...
			$client_name = stripslashes(cyr_str_decode($attrs['name']));
			//поиск по наименованию, если нашли, значит есть с таким названием, а ИНН другой - ошибка
			$client_ref = $v8->Справочники->Контрагенты->НайтиПоНаименованию($client_name,TRUE);
			if (!$client_ref->Пустая()){
				throw new Exception('Уже есть контрагент с таким наименование, ИНН:'.$client_ref->ИНН);
			}			
		
			$obj = $v8->Справочники->Контрагенты->СоздатьЭлемент();
			$obj->Наименование					= $client_name;
			...
			if(isset($attrs['okpo']) && $client_ref->КодПоОКПО<>$attrs['okpo']){
				if(is_null($obj)){
					$obj = $client_ref->ПолучитьОбъект();
				}
				$obj->КодПоОКПО=$attrs['okpo'];
			}
			if(isset($attrs['kpp']) && $client_ref->КПП<>$attrs['kpp']){
				if(is_null($obj)){
					$obj = $client_ref->ПолучитьОбъект();
				}
				$obj->КПП=$attrs['kpp'];
...
//throw new Exception('!!!ИНН='.$client_ref->ИНН);
	...
	function order($v8,$head,$items){		
		...
		if (isset($head['firm_bank_acc_ext_id'])){
			$acc_id = $v8->NewObject('УникальныйИдентификатор',$head['firm_bank_acc_ext_id']);
			$acc_ref = $v8->Справочники->БанковскиеСчетаОрганизаций->ПолучитьСсылку($acc_id);
		}
		else{
	...
	function sale($v8,$head,$items){						
		...
		*/
		...
		if ($head['pay_cash']!='t' && $calcNDS){
			...
			$doc_inv->Руководитель		= 0;
			$doc_inv->ГлавныйБухгалтер	= 0;
