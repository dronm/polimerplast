-- DROP FUNCTION public.doc_order_totals(int4, int4, int4, int4, int4, int4, numeric, int4, bool, bool, bool, bool, bool, numeric);

/**
 * called from DOCOrder_Controller->calc_totals()
 */
...
AS $function$
	...
	SELECT	
		...
		WHEN in_price_round THEN --calc fact price: total/quant
			round(
				ceil(
					CASE
				...
				/ round( (SELECT t.base_quant FROM product_params t) ,6)
			,2)
		WHEN in_price_edit THEN in_price
		ELSE
