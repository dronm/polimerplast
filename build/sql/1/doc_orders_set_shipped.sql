...
DECLARE
	...
	--
	v_total_tot numeric;
	v_total_deliv_tot numeric;
	v_total_pack_tot numeric;
...
BEGIN
	...
	LOOP
		...
		IF (r.quant_confirmed_base_measure_unit < r.quant_base_measure_unit) THEN
			...
			v_total_pack = round(coalesce(r.total_pack,0) * v_new_prod_k,2);
			v_total_deliv = round(coalesce(r.total_deliv,0) * v_new_prod_k,2);
			...
			VALUES (
				...
				CASE
					...
					ELSE (v_total - v_total_deliv ) / v_base_quant
				END,
				v_total - v_total_deliv
			);
			
			v_total_tot = v_total_tot + v_total;
			v_total_pack_tot = v_total_pack_tot + v_total_pack;
			v_total_deliv_tot = v_total_deliv_tot + v_total_deliv;
		END IF;
		...
		ELSE
			...
			-- строке скорректировать итоги в сумме с новым документом
			IF r.quant_confirmed_base_measure_unit < r.quant_base_measure_unit THEN
				IF v_total_tot <> r.total THEN
					v_total = v_total + r.total - v_total_tot;
				END IF;
				IF v_total_pack_tot <> r.total_pack THEN
					v_total_pack = v_total_pack + r.total_pack - v_total_pack_tot;
				END IF;
				IF v_total_deliv_tot <> r.total_deliv THEN
					v_total_deliv = v_total_deliv + r.total_deliv - v_total_deliv_tot;
				END IF;				
			END IF;
		END IF;
