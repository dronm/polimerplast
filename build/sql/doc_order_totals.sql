...
CREATE OR REPLACE FUNCTION public.doc_order_totals(warehouse_id integer, client_id integer, product_id integer, mes_l integer, mes_w integer, mes_h integer, quant numeric, measure_unit_id integer, pack boolean, pack_in_price boolean, to_third_party_only boolean)
  RETURNS record AS
$BODY$
	WITH
		product_params AS (
	...
	SELECT			
		...
		--ЦЕНА БЕЗ ТР - чистая		
		round(--Цена на продукцию + доп.цена за нестандартные размеры + упаковка
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.doc_order_totals(integer, integer, integer, integer, integer, integer, numeric, integer, boolean, boolean, boolean)
