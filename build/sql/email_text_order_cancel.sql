-- Function: email_text_order_cancel(order_id integer)

--DROP FUNCTION email_text_order_cancel(integer);

CREATE OR REPLACE FUNCTION email_text_order_cancel(integer)
  RETURNS RECORD AS
$BODY$
	WITH 
		templ AS (
		...
		WHERE t.email_type='order_cancel'
		)	
	SELECT
		sms_templates_text(
			ARRAY[
			...
			/*
			format('("number","%s")',
				o.number::text)::template_value,
			format('("user","%s")',
				u.name_full::text)::template_value,
			format('("firm","%s")',
				replace(f.name::text,'"','`'))::template_value,
			format('("client","%s")',
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION email_text_order_cancel(integer) OWNER TO polimerplast;
